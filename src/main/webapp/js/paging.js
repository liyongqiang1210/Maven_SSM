(function($, window, document) {
    ...
    Paging.prototype = {
        ...
        createDom: function() {
            ...
            that.options.jump ? jumpDom = '<input type="text" placeholder="1" class="jump-text" id="jumpText"><button type="button" class="jump-button btn btn-default" id="jumpBtn">跳转</button>' : jumpDom = '';
            content = '<button type="button" id="firstPage" class="turnPage first-page  btn btn-default">首页</button>' +
                '<button class="turnPage  btn btn-default" id="prePage">上一页</button>' +
                '<div class="pageWrap" style="width:' + wrapLength + 'px">' +
                '<ul id="pageSelect" style="transition:all ' + that.options.slideSpeed + 'ms">' +
                ulDom +
                '</ul></div>' +
                '<button class="turnPage btn btn-default" id="nextPage">下一页</button>' +
                '<button type="button" id="lastPage" class="last-page  btn btn-default">尾页</button>' +
                jumpDom +
