(function($, window, document, undefined) {
	...
	Paging.prototype = {
		...
		/**
		 * 页面样式定义规则： 如果current == i的话代表当前页数被选中，那么它的class应该是current-select
		 * 如果选中的页数是第一页的话，首页与上一页的按钮属性设置为禁用， 如果选中的页数是最后一页的话，下一页与尾页的按钮属性设置为禁用
		 */
		creatHtml : function() {
			var me = this;
			var content = "";
			...
			if (total > 6) {
				...
				if (current < 5) {
					for (var i = 1; i < 6; i++) {
						...
						if (current == i) {
							content += "<a class='current-select'>" + i
									+ "</a>";
						} else {
			...
			content += "<span class='totalPages'> 共<span>" + total
					+ "</span>页 </span>";
			content += "<span class='totalSize'> 共<span>" + totalNum
					+ "</span>条记录 </span>";
